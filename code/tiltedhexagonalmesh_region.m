function [] = tiltedhexagonalmesh_region (i, theta)
% Le maillage cartesien de base est de taille 2^(i+2)*2^(i+2)
% On le deforme avec une rotation d'angle theta

% Creation du fichier de maillage
file_nbr = int2str(i);
angle_str = int2str(pi/theta);
file_name = strcat('pi',angle_str,'_tiltedhexagonal_region_',file_nbr,'.typ1');
...
for k=1:(size+1)
    if mod(k,2)==1
        j = 0;
        while j<size
            nodes_coor = [nodes_coor; transformation((j+3)*h,(k-1)*h,theta) (k-1)*h; transformation((j+4)*h,(k-1)*h,theta) (k-1)*h];
            j = j+4;
        end
    else
        j = 0;
        while j<size
            nodes_coor = [nodes_coor; transformation((j+1)*h,(k-1)*h,theta) (k-1)*h; transformation((j+2)*h,(k-1)*h,theta) (k-1)*h];
            j = j+4;
        end
...
for k=1:bnd_faces_nbr
    region = 10;
    x1 = nodes_coor(faces_id_bnd(k,1),:);
    x2 = nodes_coor(faces_id_bnd(k,2),:);
    if(x1(2) == x2(2))
        if(x1(2) == 0) region = 20; elseif(x1(2) == 1) region = 40; end
    elseif(x1(1) == x2(1))
        if(x1(1) == 0) region = 10; elseif(x1(1) == 1) region = 30; end
    end
    fprintf(fid,'%i\t %i\t %i\n',faces_id_bnd(k,:), region);
end
fprintf(fid,'%s\n','all edges');
fprintf(fid,'%i\n',faces_nbr);
for k=1:faces_nbr
